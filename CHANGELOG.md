...
- Bumped the Home Assistant add-on version to `0.1.1-beta.24` and set the
  ChatGPT-account Codex OAuth default model to `gpt-5-codex`.
- Corrected repository metadata URLs to the actual beta add-on repository.
...
- Fixed Codex SSE parsing for wrapped `response.completed` events and text
  fallback events so successful calls no longer render as `No response from agent.`
- Normalized stale `gpt-5.2-codex` / `gpt-5.3-codex` / `gpt-5.5-codex`
  config values to `gpt-5-codex` at runtime.
